...
class ChatApp(ctk.CTk):
    ...
    def run_agentic_workflow(self, query):
        try:
            ...
            if docs:
                relevant = FAISS.from_documents(RecursiveCharacterTextSplitter(chunk_size=1000, chunk_overlap=200).split_documents(docs), self.embeddings).as_retriever(search_kwargs={"k": 5}).invoke(query)
    ...
    def upload_files(self):
        if files := filedialog.askopenfilenames():
            ...
            self.update_source_list(); self.status_indicator.configure(text="🟢 Dual-Agent Core Ready")
