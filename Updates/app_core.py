...
class GradientFrame(tk.Canvas):
    ...
    def _draw_gradient(self, event=None):
        ...
        for i in range(height):
            ...
            self.create_line(0, i, width, i, tags=("gradient",), fill=f"#{nr>>8:02x}{ng>>8:02x}{nb>>8:02x}")
        self.lower("gradient")
...
class LoginGateway(ctk.CTkToplevel):
    ...
    def connect(self):
        ...
        try:
            requests.post(f"{self.master_app.api_base}/register/{usr}", timeout=3)
...
class SourceAgentClient(ctk.CTk):
    def __init__(self):
        super().__init__()
        self.title("Source Agent | Nexus V33 Auto")
    ...
    def setup_ui(self):
        ...
        self.chat.insert("1.0", "SYSTEM: Autonomous Network Established.\nCOMMANDS: /announce [msg] or /msg [user] [msg]\n\n")
        self.chat.configure(state="disabled")
        
        self.entry = ctk.CTkEntry(main_frame, height=55, placeholder_text="Ask the AI, or use network commands...", font=("Segoe UI", 15))
    ...
    def save_settings(self, _=None):
        with open(SAVE_FILE, "w") as f:
            json.dump({"api_key": self.api_key, "username": self.username, "ai_model": getattr(self, "model_menu", None) and self.model_menu.get() or self.ai_model, "persona": getattr(self, "persona_menu", None) and self.persona_menu.get() or self.persona}, f)
