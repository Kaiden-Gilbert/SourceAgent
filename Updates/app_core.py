...
2) If a specific situation is described, a plain English application of the policy to that situation.

Prioritise legal/regulatory risks and order output by relevance and risk level.
If no relevant policy is found, ask 2 clarifying follow-up questions before stating none was found.

DO NOT invent, infer, or generalize. NO personal opinions or external knowledge.

Response Structure:
- Policy Reference(s): [Name + section]
- Verbatim Quote(s): [Excerpt(s)]
- Plain English Explanation: [Explanation]
- Application to Scenario: [Application]

Tone: Professional, plain, and readable. Never fabricate language."""

class PolicyAdvisorV14(ctk.CTk):
    def __init__(self):
        super().__init__()
    ...
    def setup_ui(self):
        ...
        ctk.CTkButton(sidebar, text="Upload Docs", command=self.add_docs, fg_color="#0ea5e9").pack(fill="x", padx=20, pady=5)
        ctk.CTkButton(sidebar, text="Database", command=self.open_manager, fg_color="#334155").pack(fill="x", padx=20, pady=5)
    ...
    def setup_ai(self, key):
        ...
        # Using Google Flash 1.5-8B as primary, Llama 3.1 8B as stable fallback
        self.llm = ChatOpenAI(base_url=base, api_key=key, model="google/gemini-flash-1.5-8b").with_fallbacks([ChatOpenAI(base_url=base, api_key=key, model="meta-llama/llama-3.1-8b-instruct")])
    ...
    def add_docs(self):
        ...
        self.rebuild_db()
        messagebox.showinfo("Done", "Policies indexed.")
    ...
    def open_manager(self):
        messagebox.showinfo("DB", f"Policies: {len(os.listdir(SOURCE_DIR))}")
    ...
    def query_ai(self, q):
        ...
        try:
            ...
            self.chat.insert("end", f"Error: {e}\n\n")
            
        self.chat.configure(state="disabled")

if __name__ == "__main__":
    app = PolicyAdvisorV14()
    app.mainloop()
