...
class SourceAgentWorkspace(ctk.CTk):
    def __init__(self):
        super().__init__()
        self.title("SourceAgent Pro v7.0")
    ...
    def draw_dynamic_background(self):
        ...
        # Smoother orb colors for the cinematic boot
        self.orbs = [
    ...
    def animate_bg(self):
        ...
        if w > 100:
            self.anim_step += 0.015 # Slightly faster, smoother wave
    ...
    def show_installer_wizard(self):
        ...
        ctk.CTkLabel(box, text="Please configure your core settings to continue.", text_color=self.text_muted).pack(pady=(0, 20))
    ...
    def animate_wave(self):
        ...
        for i, (lbl, basex) in enumerate(self.title_chars):
            ...
            color = self.accent if math.sin(self.wave_step + i) > 0 else self.text_main
            lbl.config(fg=color)
    ...
    # ==========================================
    # STAGE 3: THE MULTI-AI FAILOVER MATRIX
    # ==========================================
    def setup_ai_failover(self, key):
        ...
        # Heavy-Duty Backup Engine (Takes over if Primary crashes/limits out)
    ...
    def build_main_ui(self):
        ...
        ctk.CTkButton(self.sidebar, text="⚙️ Manage Sources", fg_color="transparent", border_width=1, border_color=self.text_muted, text_color=self.text_main, command=self.open_source_manager).pack(fill="x", padx=20, pady=5)
        
        self.source_count_lbl = ctk.CTkLabel(self.sidebar, text="0 Sources Loaded", font=("Segoe UI", 11), text_color=self.text_muted)
        self.source_count_lbl.pack(pady=(0, 20))
        
        # Sessions
        ctk.CTkButton(self.sidebar, text="+ New Session", fg_color=self.accent, command=self.start_new_session).pack(fill="x", padx=20, pady=10)
        ...
        ctk.CTkButton(input_bar, text="Send", width=80, command=self.send_message, fg_color=self.accent).grid(row=0, column=2, padx=10)
        
        # Status Bar
        status_frame = ctk.CTkFrame(self.chat_frame, fg_color="transparent")
        status_frame.grid(row=1, column=0, sticky="ew", padx=30)
        self.status_bar = ctk.CTkLabel(status_frame, text="🟢 Systems Online", font=("Segoe UI", 12), text_color=self.text_muted)
        self.status_bar.pack(side="left")
        ctk.CTkButton(status_frame, text="⚙️ Advanced Settings", width=60, fg_color="transparent", hover_color="#1a1a2e", text_color=self.text_muted, command=self.open_settings_menu).pack(side="right")
    ...
    # ==========================================
    # STAGE 5: ADVANCED SOURCE MANAGEMENT (RAG)
    # ==========================================
    ...
    def open_source_manager(self):
        ...
        ctk.CTkLabel(win, text="Active Database Sources", font=("Segoe UI", 20, "bold"), text_color=self.text_main).pack(pady=(20, 10))
        ...
        def refresh_list():
            ...
            if not files:
                ctk.CTkLabel(scroll, text="No sources loaded.", text_color=self.text_muted).pack(pady=20)
                return
            for f in files:
                row = ctk.CTkFrame(scroll, fg_color="transparent")
                row.pack(fill="x", pady=5)
                ctk.CTkLabel(row, text=f, font=("Segoe UI", 12), text_color=self.text_main).pack(side="left", padx=10)
                ctk.CTkButton(row, text="🗑️", width=30, fg_color="#e74c3c", hover_color="#c0392b", 
                              command=lambda filename=f: self.delete_source(filename, refresh_list)).pack(side="right", padx=10)
        refresh_list()

    def delete_source(self, filename, callback):
        try:
            os.remove(os.path.join(SOURCE_DIR, filename))
            self.status_bar.configure(text=f"Removed {filename}. Rebuilding index...", text_color="#f39c12")
            callback()
            threading.Thread(target=self.rebuild_vectorstore, daemon=True).start()
    ...
    def rebuild_vectorstore(self):
        docs = []
        # Clear old index to prevent ghost data
        index_path = os.path.join(SOURCE_DIR, "faiss_index")
        if os.path.exists(index_path):
        ...
        if docs:
            ...
            self.after(0, lambda: self.status_bar.configure(text="Index Rebuilt Successfully.", text_color="#2ecc71"))
        else:
            self.cached_vectorstore = None
            self.after(0, lambda: self.status_bar.configure(text="Source Database Empty.", text_color=self.text_muted))
    ...
    def update_source_count(self):
        ...
        if hasattr(self, 'source_count_lbl'):
            self.source_count_lbl.configure(text=f"{count} Sources Loaded", text_color="#2ecc71" if count > 0 else self.text_muted)

    # ==========================================
    # STAGE 6: CHAT & ENGINE LOGIC
    # ==========================================
    ...
    def process_query(self, query):
        try:
            self.after(0, lambda: self.status_bar.configure(text="🧠 Searching Sources...", text_color=self.accent))
            context = ""
            ...
            self.after(0, lambda: self.status_bar.configure(text="⚡ Executing Failover Matrix...", text_color=self.accent))
            ...
            self.after(0, lambda: self.status_bar.configure(text="🟢 Systems Online", text_color=self.text_muted))
            ...
            self.after(0, lambda: messagebox.showerror("Matrix Error", f"Both Primary and Backup AI failed.\n\n{str(e)}"))
            self.after(0, lambda: self.status_bar.configure(text="🟢 Systems Online", text_color=self.text_muted))

    # ==========================================
    # STAGE 7: SESSION & APP SETTINGS
    # ==========================================
    ...
    def open_settings_menu(self):
        ...
        # Nuclear Option
        def nuke_history():
            if messagebox.askyesno("Confirm Data Wipe", "Are you sure you want to permanently delete all chat history?"):
                for f in os.listdir(HISTORY_DIR): os.remove(os.path.join(HISTORY_DIR, f))
                self.session_history = []
                self.save_current_state()
                self.start_new_session()
                self.update_sidebar_history()
                messagebox.showinfo("Success", "All chat history purged.")
                
        ctk.CTkButton(win, text="🧨 Wipe All Chat History", fg_color="#e74c3c", hover_color="#c0392b", command=nuke_history).pack(pady=20)
